...
function Save-DryDependenciesHash {
    [CmdLetBinding()]
    param (
        [Parameter(Mandatory)]
        [String]$Path,
    ...
    try {
        [String]$Hash = Get-DryDependenciesHash -Dependencies $Dependencies
        [PSObject]$DependencyObject = Get-Content -Path $Path -Encoding default -ErrorAction Stop | 
        ConvertFrom-Json -ErrorAction Stop

        $DependencyObject.dependencies_hash = $Hash
