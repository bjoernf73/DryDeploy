Function dry.action.dsc.run {
    ...
    Try {
        ...
        [String]$DscMetaConfigFile   = (Get-ChildItem -Path "$ConfigSourcePath\*" -Include *.json,*.jsonc -ErrorAction Stop).FullName
        [String]$DscTemplateFile     = (Get-ChildItem -Path "$ConfigSourcePath\*" -Include *.psm1 -ErrorAction Stop).FullName

        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
        #   METACONFIG
        #
        #   The MetaConfig is a configfile with info about the actual DSC config
        #
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #  
        ...
        #   The DSC Template
        #
        #   The DSC Template is an incomplete DSC psm1-file. It lacks the specific 
        #   modules and resources to install and load. This is merely because: I could 
        #   put them in the psm1, but DSC, for some reason, does not handle the installation
        #   of them on the local, and target system. So they're in the MetaConfig
        #
        # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
        [String]$DscTemplate = Get-Content -Path $DscTemplateFile -Raw -ErrorAction Stop
        ...
        ol i @("The DSC Target is","$DSCTarget")
        ...
        If ($MetaConfigObject.dsc_modules) {
            ...
            ol i @('Trying to install DSC modules on',"$DSCTarget")
        ...
        If ($Action.credentials.credential2) {
            ...
            Else {
                ol i @('Disallowing alternating credentials. Using',"'$($Action.credentials.credential2)'")
        ...
        If ($VerificationObject) {
            ol i @('DSC verification log events','Yes')
